...
Note that the data are indeed from the null as the difference between the two population means is zero. The variable muGlobal can take on any value under the null, and is referred to as a nuisance parameter. In this case both the null and the alternative hypothesis/model are composite. 
...
A p-value analysis relies on the sampling distribution of a test statistic. The natural statistic to consider here is the difference between the sample means. The sample mean $\bar{X}_{1}$ has sampling distribution 
...
The previous code worked for a specific data set/outcome of a single experiment. Note that this 1 data set yielded 100 p-values. We now repeat the procedure, but over 1,000 data sets/experiments. The code below stores 1,000 times 100 p-values in the variable `allPValues`. 

We will see that far more than 50 out of these 1,000 data sets/repeated experiments will yield a "significant" result, despite the data being sampled under the null. 
...
```
The reason why type I error over-inflates is due to classical p-value methods being developed for fixed designs. For these designs, the sample size at which both data collection is stopped, and analysed needs to be fixed in advanced. Deviating from this plan results in multiple looks at the data resulting in an over-inflation of the type I error. The plot emphasises this as it shows that upon the first look the procedure already has a type I error of alpha. The use of classical p-values thus confines researchers to analysing the data once, and only once. 
...
```
This Bayes factor sample path exceeds both evidence thresholds of 20 (1/alpha) and 30 ("strong evidence"). We will see in general this procedure yields a type I error  larger than alpha. 
...
lines(c(1, n1), c(5, 5), lwd=2, lty=2)
```
The broken brown curve corresponds to rejecting the null as soon as bf10 > 30, whereas the full brown curve corresponds to bf10 > 1/alpha=20. Regardless of the threshold for the Bayes factor, the type I error is much higher than 5\%. 
...
lines(c(1, n1), c(5, 5), lwd=2, lty=2)
```
Hence, after n1=n2=100 observations, i.e. looks at the data, we falsely reject 26 out of the 1,000 times the null (2.6\%). This is well below the tolerable alpha of 5\%. The type I error control that e-values provide is independent of the maximum sample size. If we increase it to n1=n2=200, we get only 2 additional false rejections resulting in a total of 28 out of 1,000 false rejections (2.8\%) still below the 5\%. One might wonder whether the chance of a false rejection will **ever** cross the tolerable 5\%. In a very general framework, Grunwald, de Heide and Koolen (2019) showed that this will not occur, even if we sample indefinitely under the null. 

The following plot summarise the false rejections rates of the three procedures after n1=n2=100 observations/looks: In red the sequential p-value procedure resulting in a type I error of 38.4\%, in brown the subjective Bayes factor procedure resulting in a type I error of 17.3\%, and the e-value procedure resulting in a type I error of 2.6\%: 
...
In the next document we show that the e-value procedure is not only safe to optional stopping, but also powerful. We also elaborate on the design of experiments with e-variables.
