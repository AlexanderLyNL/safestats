...
For the p-value demonstration we use the `pValueZTest` function from the safestats package. The details provided here are only included for to those particularly interested in its derivation. This subsection can be safely skipped, as the use of the `pValueZTest` function becomes clear in the next subsections.
...
lines(c(1, n1), c(5, 5), lwd=2, lty=2)
```
Hence, after n1=n2=100 observations, i.e. looks at the data, we falsely reject 26 out of the 1,000 times the null (2.6\%). This is well below the tolerable alpha of 5\%. 

We would like to add two comments. Firstly, the type I error control that e-values provide is independent of *any* maximum sample size. If we increase it to n1=n2=200, we get only 2 additional false rejections resulting in a total of 28 out of 1,000 false rejections (2.8\%) still below the 5\%. One might wonder whether the chance of a false rejection will **ever** cross the tolerable 5\%. In a very general framework, Grunwald, de Heide and Koolen (2019) showed that this will not occur, even if we sample indefinitely under the null. 

Secondly, type I error control holds for any hyper parameter g that was set by design object. The next document provides additional information regarding the choice of g. 
...
In the next document we show that the e-value procedure is not only safe to optional stopping, but also powerful.
