...
The working example will be a two-sample z-test. Suppose we have historical data showing that incoming first-year psychology students in the Netherlands have an average IQ score of 107. The population standard deviation is assumed to be known to be 2. A new educational programme is developed and it claims to increase IQ scores by 10 points. A total of n1 students are assigned to the treatment group, and n2 students are assigned to the control group. The null and alternative hypotheses are respectively given by 
...
Note that the data are indeed from the null as the difference between the two population means is zero. Hence, the variable muGlobal can take on any value under the null, and it was arbitrarily set higher than the historical  107, due to students getting more intelligent over time in general. 
...
for (i in 1:n1) {
  pValueVector[i] <- pValueZTest(x=dataGroup1[1:i], y=dataGroup2[1:i],
                                 sigma=sigmaTrue)$pValue
}

oldPar <- setSafeStatsPlotOptionsAndReturnOldOnes();

plot(1:n1, pValueVector, type="l", lwd=2, xlab="n", ylab="p-value")
...
oldPar <- setSafeStatsPlotOptionsAndReturnOldOnes();
plot(1:n1, pValueFdr, type="l", xlab="n", ylab="Type I error", lwd=2)
lines(c(1, n1), c(0.05, 0.05), col="red", lwd=2, lty=2)
```
...
The hyper parameters are set based on historical knowledge. 
...
## Sequential analysis
The sequential use of the subjective Bayes factor entails computing a Bayes factor after each observation, and claiming an effect, as soon as the Bayes factor bf10 in favour of the alternative over the null is above the evidence threshold of 1/alpha, say, 20 for alpha=0.05. This procedure is not safe, as it will lead to false rejections with more than 5\% chance. The algorithmic description of a sequential subjective Bayes factor  analysis is as follows: 
...
### One sample path
We demonstrate this procedure for a single data set of n1=n2=100 samples, and we monitor the subjective Bayes factor a 100 times, after an observation of each group.
...
plot(1:n1, bfVector, type="l", lwd=2, xlab="n", ylab="BF10", col="brown")
lines(c(1, n1), c(1/alpha, 1/alpha), col="red")
```
This sample path does not exceed the evidence threshold of 20, but, as we will see in the next subsection, as a procedure this subjective Bayes factor yields type I error larger than alpha. 
...
oldPar <- setSafeStatsPlotOptionsAndReturnOldOnes();
plot(1:n1, subjectiveBfValueFdr, type="l", xlab="n", ylab="Type I error", 
     lwd=2, col="brown", ylim=c(0, 0.35))
lines(c(1, n1), c(0.05, 0.05), col="red", lwd=2, lty=2)
```
...
plot(1:n1, allBfValues[someIndex, ], type="l", lwd=2, xlab="n",
     ylab="BF10", col="brown")
lines(c(1, n1), c(1/alpha, 1/alpha), col="red", lwd=2, lty=2)
```

# 3. <a name = "eValues">Sequential use of a safe test/e-variable will **not** over-inflate the type I error</a>

For the e-variable demonstration we rely on the `safeZTest` function from the safestats package. This e-variable can be thought of as a Bayes factor with specifically chosen priors so it can be safely used sequentially. 

To run the `safeZTest` a design object needs to be created. This design object can exploit knowledge of a minimal clinically relevant effect size to speed up inference, say, 0.5. 

```{r, eval=FALSE}
designObj <- designSafeZ(meanDiffMin=0.5, testType="twoSample",
                         sigma=sigmaTrue)
designObj
```

## Sequential analysis
The sequential use of e-variables entails computing an e-value after each observation, and claiming an effect, as soon as the e-value in favour of the alternative over the null is above the evidence threshold of 1/alpha, say, 20 for alpha=0.05. This procedure is not safe, as it will **never** lead to a false rejection rate of more than 5\%. The algorithmic description of the sequential e-value procedure is as follows: 
...
plot(1:n1, eValueVector, type="l", lwd=2, xlab="n", ylab="eValue", 
     col="blue")
lines(c(1, n1), c(1/alpha, 1/alpha), col="red", lwd=2, lty=2)
```
This sample path does not exceed the evidence threshold of 20, and fewer than 5\% of experiments will do, even if we increase the sample size indefinitely. 
...
plot(1:n1, pValueFdr, type="l", xlab="n", ylab="Type I error", 
     bty="n", lwd=2, ylim=c(0, 0.35))
lines(c(1, n1), c(0.05, 0.05), col="red", lwd=2, lty=2)
lines(1:n1, subjectiveBfValueFdr, col="brown")
lines(1:n1, eValueFdr, col="blue")
```
It is worth emphasising that the false rejection rate of the e-variable will slowly increase as the end point, here n=100, increase, but it will never be larger than alpha. 
