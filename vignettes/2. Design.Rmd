---
title: "Experimental design: Behaviour of e-variables under alternatives"
author: "Alexander Ly"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{workshop2Design}
...
In the previous markdown document we studied the behaviour of three procedures under the null: The sequential use of (1) p-values, (2) subjective Bayes factor, and (3) e-variables. 

We saw that all allow us to stop experiments early, but that for the first two procedures this come with the high cost of drawing the wrong conclusion that the null should be rejected. On the other hand, we saw that with e-variables we do retain type I error control. 

In this document we consider the behaviour of the e-variable under the alternative. We elaborate on the following: 

- The design of experiments based on e-variables
- How we can stop early if there truly is an effect

We elaborate these ideas with the running example of the two-sample z-test discussed in the previous document. 

# 1. <a name = "designT">Designing safe t-test experiments</a>
## Designing experiments

In principle, we can continue sampling and monitor the e-value until we find compelling evidence, i.e. e > 1/alpha, say, alpha=0.05. This, however, might not be practical, as our time, money, and effort for doing research are limited. 

To get an idea of how long to sample for, we can run the following code that additional to alpha considers (1) the minimal clinically relevant mean difference, and (2) a targeted power of  1-beta=80\%, or equivalently a a tolerable type II of beta=0.2. For the example at hand with a claimed increase 10 IQ scores, we can set meanDiffMin=10. 
...
In the first case, we mean by the design of experiments the (planned) sample size need

The general behaviour of e-variables is as follows: (1) if the null holds true, then the e-value will slowly decrease to zero, whereas (2) if the alternative holds true, it will accumulate evidence against the null exponentially fast.

Provided with a minimal clinically relevant (1) mean difference, (2) tolerable type II error (i.e. beta), or equivalently, power (i.e. 1-beta), we would like to know the sample size to plan for. If the sample size needed to plan for such an experiment requires too much time, funds, or effort, we might decide to spent our time on more reachable inference instead. 



we would 

Before we engage in experimentation it is good to 
...
designObj <- designSafeZ(meanDiffMin=meanDiffMin,
                         alpha=alpha,
                         beta=beta, testType="twoSample",
                         seed=1, pb=TRUE)
```
```{r}
designObj
