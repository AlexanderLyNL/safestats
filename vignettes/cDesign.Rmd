...
1. In the [first section](#designZ) we clarify how the savi design function determines both the parameter gMom and the planned sample size, if it is given a minimal clinically relevant effect size, and a targeted power/tolerable type II error rate, respectively.  
...
# 1. <a name = "designZ">Designing savi z-test experiments</a>
## One sample path
Recall the example described in the installation guide with a minimal clinically relevant mean difference of 10 (IQ score increase) in a population with standard deviation sigma=12. In other words, a minimal clinically relevant (standardised) effect size of 10/12 = 0.83. 

To sequentially detect such a minimal clinically relevant mean difference with 80\% chance/power, we have to __plan__ an experiment with about 36 participants in both the treatment and the control group: 
