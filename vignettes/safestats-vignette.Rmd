...
In the previous section we saw that monitoring the p-value and stopping before the planned sample sizes whenever $p < \alpha=0.05$ leads to an increased risk of a false claim (from 5% to `r simResultDeltaTrueIsZero$freqSim$powerOptioStop*100`%). 
...
Hence, after a first batch of data, we get `r sum(pValuesBatch1 < alpha)` incorrect null rejections out of a 1,000 experiments (`r mean(pValuesBatch1 < alpha)*100`%). 

The following code continues only the non-significant `r round(mean(pValuesBatch1 > alpha)*1000)` experiments with a second batch of data all also generated under the null, and plots two histograms. <!--In blue the histogram of all the non-significant results (954 experiments), and in red the histogram of p-values over the combined data set. -->
...
thus, `r sum(eValuesBatch1 > 1/alpha)` false null rejections out of 1,000 experiments. 
...
show that `r sum(eValuesBatch1To2 > 1/alpha)` out of the `r length(eValuesBatch1To2)` of the selectively continued experiments (`r mean(eValuesBatch1To2 > 1/alpha)*100`%) now lead to a false null rejection due to optional continuation. Hence, after the second batch of data the total number of total number of false null rejections is `r sum(eValuesBatch1 > 1/alpha) + sum(eValuesBatch1To2 > 1/alpha)` out of a total of a 1,000 original experiment, thus, `r sum(eValuesBatch1 > 1/alpha) + sum(eValuesBatch1To2 > 1/alpha)/10`%. Similar, a third batch won't increase the collective false rejection rate either. 
...
The follow-up batch of data will again be of the same sizes, thus, `r designObj$nPlan`, and be generataed with the same deltaTrue smaller than deltaMin, as in the first batch. 
...
The following code selectively continues the `r 1000-sum(eValuesBatch1 > 1/alpha)` experiments which did not lead to a null rejection:
...
an additional `r sum(eValuesBatch1To2 > 1/alpha)` correct null rejections (`r sum(eValuesBatch1To2 > 1/alpha)/sum(eValuesBatch1 < 1/alpha)*100`% of `r sum(eValuesBatch1 < 1/alpha)` experiments). This brings up the total number of null rejections to `r sum(eValuesBatch1To2 > 1/alpha) + sum(eValuesBatch1 > 1/alpha)` out of the 1,000 experiments. In this case, a null rejection is correct, since the data were generated with a true effect larger than zero. 
...
```
This led to `r sum(eValuesMulti > 1/alpha)` null rejections out of the 1,000 experiments, which is the correct result as the effect is present in both the original and replication studies. 
