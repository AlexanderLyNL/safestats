...
_Safe tests_ is a collective name for a new form of hypothesis tests that yield _e-values_ (instead of _p-values_). The original paper on e-values by Grünwald, de Heide and Koolen can be found [here](https://arxiv.org/abs/1906.07801). For each hypothesis testing setting where one would normally use a $p$-value, a safe test can be designed, with a number of advantages that are elaborately described and illustrated in this vignette. Currently, this package provide e-values for the t-test, Fisher's exact test, and the chi-squared test (the safe test of 2 proportions). In this vignette, we will illustrate the concepts of safe testing and e-values through the t-test example. These safe tests were designed to be _GROW_; they perform the best under the worst case in the alternative hypothesis.
