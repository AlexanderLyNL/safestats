...
_Safe tests_ is a collective name for a new form hypothesis tests that are based on _s-values_ (instead of _p-values_). S-values are all random variables that have an expected value of at most one under the null hypothesis; the original paper on s-values by Grünwald, de Heide and Koolen can be found [here](https://arxiv.org/abs/1906.07801). For each hypothesis testing setting where one would normally use a p-value, an s-value could be designed, with a number of advantages that are elaborately described and illustrated in this vignette. Currently, in this package, s-values that are alternatives to the t-test and Fisher's exact test and the Chi-squared test (the safe test of 2 proportions) are available. These s-values were designed to be _GROW_: they are the s-values that, for each specific hypothesis testing setting, perform the best under the worst case in the alternative hypothesis (see the [original paper](https://arxiv.org/abs/1906.07801)).
