...
Thus, in this simulation with the null hypothesis holding true and if the safe test is only conducted once at the planned sample size, then in `sum(eValues > 20)` out of 1,000 experiments the null hypothesis was falsely rejected. 
...
```
Note that optional stopping always increases the chance of observing a false detection. For the safe test this increased to `simResultDeltaTrueIsZero$safeSim$powerOptioStop*100`%, which is still below the tolerable 5%. On the other hand, tracking the $p$-value and rejecting the null as soon it falls below $\alpha$ leads to `simResultDeltaTrueIsZero$freqSim$powerOptioStop*100`%, which is well above 5%. 
...
```
The simulations confirms that at the planned sample size there is about 80% chance of detecting the minimal clinically relevant effect. The discrepancy of 0.3% is due to sampling error and vanishes as the number of simulations increases. 
...
tabResult
```
The table shows the full distribution of the times at which the experiment is stopped. For instance, `sum(tabResult[1:which(names(tabResult)==floor(designObj$nPlan[1]/2))])` out of the 1,000 experiments stopped before half the planned sample size. In these cases we were lucky and the effect was detected early. The last bar collects all experiments that ran until the planned sample sizes, thus, also those that did not lead to a null rejection at n=`designObj$nPlan[1]`. To see the distributions of stopping times of only the experiments where the null is rejected, we run the following code:
...
In the previous section we saw that monitoring the $p$-value and stopping before the planned sample sizes whenever $p < \alpha=0.05$ leads to an increased risk of a false claim (from 5% to `simResultDeltaTrueIsZero$freqSim$powerOptioStop*100`%). 
...
Hence, after a first batch of data, we get `sum(pValuesBatch1 < alpha)` incorrect null rejections out of a 1,000 experiments (`mean(pValuesBatch1 < alpha)*100`%). 

The following code continues only the non-significant `round(mean(pValuesBatch1 > alpha)*1000)` experiments with a second batch of data all also generated under the null, and plots two histograms. <!--In blue the histogram of all the non-significant results (954 experiments), and in red the histogram of $p$-values over the combined data set. -->
...
show that by extending the non-significant results of the first batch with a second batch of data, we got another `sum(pValuesBatch1To2 < alpha)` false null rejections. This brings the total number of incorrect null rejections to `sum(pValuesBatch1 < alpha)+sum(pValuesBatch1To2 < alpha)` out of 1,000 experiments, hence, `(sum(pValuesBatch1 < alpha) + sum(pValuesBatch1To2 < alpha))/10`%, which is above the tolerable $\alpha$-level. 
...
In this subsection, we show that only continuing studies for which the $e$-value $\leq 1/ \alpha$ does *not* lead to an overrejection of the null. This is because the sampling distribution of $e$-values under the null slowly drifts towards smaller values as the number of samples increases. 
...
show that `sum(eValuesBatch1To2 > 1/alpha)` out of the `length(eValuesBatch1To2)` of the selectively continued experiments (`mean(eValuesBatch1To2 > 1/alpha)*100`%) now lead to a false null rejection due to optional continuation. Hence, after the second batch of data the total number of total number of false null rejections is `sum(eValuesBatch1 > 1/alpha) + sum(eValuesBatch1To2 > 1/alpha)` out of a total of a 1,000 original experiment, thus, `sum(eValuesBatch1 > 1/alpha) + sum(eValuesBatch1To2 > 1/alpha)/10`%. Similar, a third batch won't increase the collective false rejection rate either. 
...
The follow-up batch of data will again be of the same sizes, thus, `designObj$nPlan`, and be generataed with the same deltaTrue smaller than deltaMin, as in the first batch. 
...
`sum(eValuesBatch1 > 1/alpha)` correct null rejections, since this simulation is based on data generated under alternative with deltaTrue > 0. 

The following code selectively continues the `1000-sum(eValuesBatch1 > 1/alpha)` experiments which did not lead to a null rejection:
...
an additional `sum(eValuesBatch1To2 > 1/alpha)` correct null rejections (`sum(eValuesBatch1To2 > 1/alpha)/sum(eValuesBatch1 < 1/alpha)*100`% of `sum(eValuesBatch1 < 1/alpha)` experiments). This brings up the total number of null rejections to `sum(eValuesBatch1To2 > 1/alpha) + sum(eValuesBatch1 > 1/alpha)` out of the 1,000 experiments. In this case, a null rejection is correct, since the data were generated with a true effect larger than zero. 
...
eValuesRep <- vector("numeric", length=1000)
...
This shows that the type I error (`mean(eValuesMultiply > 1/alpha)*100`% < $\alpha=5$%) is still controlled for, even if the replication attempt is done on a different population. In fact, the $\alpha$-level is controlled for regardless of the values of the nuisance parameters (e.g., $\mu_{g}$ and $\sigma$), or the sample sizes of the replication attempt as long as there are more than 2 data points in any study. 
...
```
This led to `sum(eValuesMulti > 1/alpha)` null rejections out of the 1,000 experiments, which is the correct result as the effect is present in both the original and replication studies. 
