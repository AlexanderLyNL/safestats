...
```
The following code replicates this setting a 1,000 times and shows that indeed, only a very few times will the sValues cross the boundary of $1/\alpha$
...
```{r}
oldSValues <- simResultDeltaTrueIsDeltaMin$safeSim$sValues
oldData <- list(dataGroup1=simResultDeltaTrueIsDeltaMin$safeSim$dataGroup1,
...
selectivelyContinueDeltaTrueIsZero <- 
  selectivelyContinueTTestCombineData(oldValues=oldSValues,
                                      ...
                                      seed=2, paired=TRUE)
```
Hence, most of the s-values that did not lead to a null rejection also do not lead to a null rejection for the combined data. Recall that optional stopping with the s-value led to 24 false rejection and that 
```{r}
sum(selectivelyContinueDeltaTrueIsZero$newValues >= 1/alpha)
length(selectivelyContinueDeltaTrueIsZero$newValues)
```
shows that 7 out of the 976 of continued experiments (0.7%) lead to a null rejection due to optional continuation. Hence, after one replication attempt the total number of total false null rejection is 31 out of a total of a 1,000 original experiment, thus, 3.1%. <!-- is still below the tolerable $\alpha$-level of 5%. -->
...
for (j in 1:3) {
  ...
  print(paste("Replication attempt", j+1))
  print(sum(selectivelyContinueDeltaTrueIsZero$newValues > 1/alpha))
}

```
and shows that the number of false null rejections decreases as the number of replication attempts increases (24, 7, 1, 0, 0, ...), and the total number of false null rejections remains well below the tolerable $\alpha$-level. 

### How optional continuation is problematic for p-values
The situation for $p$-values however is different, as the number of additional rejections do not decrease with the number of replications attempts. As optional stopping is not adviced for $p$-values, we restart with a total of 1,000 experiments, all ran as planned:

```{r}
oriData <- generateTTestData(n1Plan=freqDesignObj$n1PlanFreq,
                             n2Plan=freqDesignObj$n2PlanFreq, 
                             deltaTrue=0, nsim=1000, paired=TRUE, seed=1)

dataGroup1 <- oriData$dataGroup1
dataGroup2 <- oriData$dataGroup2

pValues <- vector("numeric", length=1000)

for (i in seq_along(pValues)) {
  pValues[i] <- t.test(x=dataGroup1[i, ], y=dataGroup2[i, ], 
                       alternative="greater", paired=TRUE)$p.value
}
mean(pValues > alpha)
sum(pValues < alpha)
```
This results in 46 null rejection out of a 1,000, hence, 4.6%. A first replication attempt shows that 

```{r}
selectivelyContinueDeltaTrueIsZeroWithP <-
  selectivelyContinueTTestCombineData(oldValues=pValues,
                                      valuesType="pValues", 
                                      alternative="greater", 
                                      oldData=oriData,
                                      deltaTrue=0,
                                      ...
                                      alpha=alpha,
                                      seed=2, paired=TRUE)
sum(selectivelyContinueDeltaTrueIsZeroWithP$newValues < alpha)
```
reveals an additional 28 false null rejections, thus, in total 74 out of 1,000 experiments, hence, 7.4%, which is already above the tolerable $\alpha$-level. 
...
As with the safe t-test, the safe test for two proportions can be used in the optional stopping setting while retaining the type I error guarantee. In the figure below the spread of the stopping times among 1,000 simulated experiments is depicted, if the real effect size is equal to the minimal clinical relevant effect size we plan for:
