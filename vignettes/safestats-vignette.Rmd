...
Based on a tolerable type I error rate of $\alpha = 0.05$, type II error rate of $\beta = 0.20$, and minimal clinical effect size of $\delta_{\min} \approx 0.42$, the following code shows that we then need to plan an experiment consisting of 63 ppatients each measured before (n1Plan) and after (n2Plan)  the treatment. 
...
# delta <- (120-120)/(sqrt(2)*15)
```

then the safe test applied to these data will be larger than $1/\alpha = 20$ with at most $\alpha =$ 5% chance
...
Hence, the simulations demonstrate how power is gained as a result of optional stopping whenever the true effect size equals the minimal clinical relevant effect size. 
...
Hence, this means that if the true effect is larger than what was planned for, the safe test will pick this larger effect up earlier on, which results in a further increase of efficiency. 

### Optional stopping does not causes safe tests to overreject the null
The previous examples highlight how optional stopping results in an increase in power, i.e., the chance of rejecting the null is increased, when the alternative is true. When the null holds true, however, the rejection rate should be low, at least not larger than the tolerable type I error rate. Here we show that optional stopping result in the type I error rate of the safe test to *not* exceed $\alpha$, whereas early stopping with classical $p$-value tests *does* result in the exceedance of the prescribed $\alpha$-level. In other words, optional stopping with $p$-values leads to an increased risk of falsely claiming that a medicine is effective, while in reality the effect is abscent. 
...
Lastly, we illustrate that the multiplication of independent s-values yields an overall s-value that is also safe. This is particularly interesting when the goal is to control the $\alpha$-level for an overall null effect in the context of replication studies.  <!-- safe tests computed over an original and replication data set can be combined by simple multiplication and that this procedure also conserves the type I error guarantee. -->
...
show that by extending the first batch with a second batch of data, we got another 28 false null rejections. This brings the total number of false null rejections to 74 out of 1,000 experiments, hence, 7.4%, which is above the tolerable $\alpha$-level. 

The reason why $p$-values overreject the null under optional continuation is due to $p$-values being uniformly distributed under the null. As such, when the null holds true and the number of samples increases, the $p$-value then meanders between 0 and 1, thus, eventually crossing any fixed $\alpha$-level. 
...
-->
### Optional continuation does not result in safe tests exceeding the prescribed $\alpha$-level
In this subsection, we show that only continuing studies for which $S \leq 1/ \alpha=20$ does *not* lead to an overrejection of the null. This is because the sampling distribution of s-values under the null slowly drifts towards smaller values as the number of samples increases. 

Again, we consider the situation in which we only continue studies for which the original s-values did not lead to a null rejection. As first batch s-values, we use the simulation study ran in the previous section, and we recall that under optional stopping we get 
...
The simulations show that the realised number of false null rejections decreases as the number of replication attempts increases (24, 7, 1, 0, 0, ...). Consequently, the collective rate of false null rejections remains well below the tolerable $\alpha$-level. The histograms slowly drifting to the left show that the chance of seeing an s-value larger than $1/\alpha$ decreases under the null. 
...
The following code selectively continues the 145 non-rejected experiments 
...
and reveals that after the second batch of data that the sampling distribution of s-values now concentrates on larger values, as is apparent from the blue histogram shifting to the red histogram on the right. Note that most red histogram's mass is on the right hand side of the grey vertical line that represents the $\alpha$ threshold (e.g., $\log(1/\alpha) \approx 3$. The continuation of the 145 experiments with $S < 1/\alpha=20$ led to 
...
### S-values for replication studies
It is not always appropriate to combine data sets. In particular, for replication attempts where the original experiment is done on a different population. In that case, one can still easily do safe inference by multiplying the s-values computed over each data set separatedly. This procedure also conserves the $\alpha$-level, as we show below. 

In all scenarios the simulation results of the optional stopping studies are used as original experiments. The data from these simulated experiments were all generated with a global population mean (e.g., baseline blood pressure) that was set to $\mu_{g}=120$, a population standard deviation of $\sigma=15$, and a deltaTrue, which depending on the absence or presence of the effect is zero, or, say, equal to deltaMin, respectively. 
...
As with the safe t-test, the safe test for two proportions can be used in the optional stopping setting while retaining the type I error guarantee. In the figure below the spread of the stopping times among 1,000 simulated experiments is depicted, if the real effect size equals the minimal clinical relevant effect size as planned:
...
Optional stopping, however, causes Fisher's exact test to overreject the null. When the null is true, the rate of incorrect null rejections exceed the tolerable $\alpha$-level:
...
In each of the simulations above, a fraction of the experiments did not lead to the rejection of the null hypothesis. Since safe tests allow for optional continuation, one could decide to plan a follow-up experiment after such a 'failed' first experiment, for example when the s-value looks promisingly high. The resulting s-values could then be multiplied to calculate a final s-value.

<!--@Rosanne, dit is een lastig zin voor mij. -->
If we study the spread of the s-values at the end of data collection in the cases where we did not reject in our first simulation, where the true means were equal to our design means, we observe the following spread:

```{r}
notRejectedIndex <- which(optionalStoppingTrueMeanIsDesign$rejected==FALSE)

sValuesNotRejected <- 
  optionalStoppingTrueMeanIsDesign$s_values[notRejectedIndex]

mainTitle <- "Spread of s-values when null not rejected: under delta design"

graphics::plot(stats::density(sValuesNotRejected), 
               xlim=c(0, max(sValuesNotRejected)), xlab="s-values", 
               main=mainTitle)
```

The spread of s-values for data under the null behave differently and show the following pattern:

```{r}
rejectedIndex <- which(optionalStoppingTrueMeanIsDesign$rejected == FALSE)

sValuesNotRejectedNull <- 
  optionalStoppingTrueMeanNull$s_values[rejectedIndex]

mainTitle <- "Spread of S-values when null not rejected: under null"

graphics::plot(stats::density(sValuesNotRejectedNull), 
               xlim=c(0, max(sValuesNotRejected)), xlab="s-values", 
               main=mainTitle)
```
