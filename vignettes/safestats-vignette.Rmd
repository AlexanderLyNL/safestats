...
Based on these plots we could conclude that studies that yielded a final S-value between 10 and 20 look promising; under the null hypothesis, such high S-values where not observed in the spread plot! What would happen if we followed these studies up with a small extra study with 40 participants, and combined the resulting S-values?
...
graphics::plot(stats::density(newSValues), xlim = c(0, max(newSValues)), xlab = "S value", main = "Spread of optional continuation S-values")
graphics::abline(v = 20, lty = 2, col = "red")
```

How many of the initially futile experiments will now lead to rejection of the null hypothesis?
```{r}
mean(newSValues>=20)
```
...
mean(newSValues>=20)
```
We still keep our type_-I error probability guarantee.
