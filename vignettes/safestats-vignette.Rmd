...
Second, simulations are run to show that safe tests indeed conserve the type I error guarantee under optional stopping. We also show that optional stopping causes the false null rejection rate of the classical $p$-value test to exceed the promised level $\alpha$ type I error guarantee. This implies that with classical tests one cannot adapt to the information acquired during the study without increasing the risk of making a false discovery. 

 - [for testing means](#optStopT)
 - [for testing proportions](#optStop2x2)

Finally, it is shown that _optionally continuing non-significant experiments_ also causes the $p$-value tests to exceed the promised level $\alpha$ type I error guarantee, whereas this is not the case for safe tests. 
...
We highlight this with our design object 
...
```{r}
# our deltaMin is: 9/(sqrt(2)*15) = 0.42
deltaTrueLarger <- 0.6
...
### Two ways to optionally continue studies with safe testing
...
### a. Optional continuation by _extending_ the sample size does not result in safe tests exceeding the tolerable $\alpha$-level
...
In one of our previous optional stopping simulations, the true mean difference between the groups was equal to our design mean difference of 0.3. In 13.2% of the simulations, we failed to reject the null hypothesis. We could now look at the spread of the s-values in these experiments:
...
It can be observed that the s-values are spread out between 0 and 13. How would this pattern behave when this experiment was carried out under the null? Below, the s-values from unrejected experiments under the null are plotted:
